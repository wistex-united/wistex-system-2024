...
To setup up this repository, first install the dependencies via apt as listed [here](https://docs.b-human.de/master/getting-started/initial-setup/), then run:


````
git clone --recursive git@github.com:Badger-RL/BadgerRLSystem.git

````

There are MacOS and Ubuntu specific instructions for generating and compiling the code [here](https://docs.b-human.de/master/getting-started/initial-setup/).
...
````
## [NEW:] Testing Policies on SimRobot

To run evaluation of trained policies in `SimRobot`, we can use the following command:

`python3 run_eval.py --rl-cfg=[RL-CFG-TYPE] --type=[SIM-RANDOMIZATION-TYPE] --iter=10` 

To see the already available configs have a look at the `rl_cfgs/` and the `eval_cfg/` folder. The randomizations are done uniformly from `[x_min, x_max]` and `[y_min, y_max]`. This will execute 10 iterations with randomization configured as per the yaml file. 

Alternatively, one can override specific arguments using the following command. This will use `rl_configs/default.cfg` and replace specific 

`python3 run_eval.py --type=[SIM-RANDOMIZATION-TYPE] --iter=10 num_robots=5 role_switch_distance=0.2`
