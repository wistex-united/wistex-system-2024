...
float scoreTime = 0;
// TESTING SEARCH
bool simTesting = false;
bool fileCreated = false;


void createDataFile(int success, float time){
    ...
    FILE* file = fopen("tempdata.txt", "w");
...
{
  scoreTime = theFrameInfo.getTimeSince(theGameState.timeWhenPlayerStateStarted)/1000;
  if (scoreTime > 60 && simTesting && !fileCreated){
    createDataFile(0, scoreTime);
    fileCreated = true;

  }

  if (theGameState.ownTeam.score > 0 && simTesting && !fileCreated) {
    createDataFile(1, scoreTime);
    fileCreated = true;
  }
  if (theGameState.opponentTeam.score > 0 && simTesting && !fileCreated) {
    createDataFile(0, scoreTime);
    fileCreated = true;
  }
  ...
  if(theGameState.isReady()){
    ...
    // std::cout << "Game state is set" << std::endl;
  ...
  {
      return SkillRequest::Builder::neuralControl();
  }

  // if out of bounds
  if (theGameState.isForOpponentTeam() && simTesting && !fileCreated){
    // Out of bounds
    createDataFile(0, scoreTime);
    fileCreated = true;
  }
// avoid calling neural control during these states
  if(theGameState.isFreeKick() || theGameState.isGoalKick() || theGameState.isCornerKick()) {
      return SkillRequest::Builder::neuralControl();
  }

  // save the global time
  // scoreTime = theFrameInfo.getTimeSince(theGameState.timeWhenStateStarted) / 1000;

  // [SimRobot training] If training a model with SimRobot, uncomment the following line to use
  // the neuralControlTraining skill.
  // return SkillRequest::Builder::neuralControlTraining();
  return SkillRequest::Builder::neuralControl();
}
