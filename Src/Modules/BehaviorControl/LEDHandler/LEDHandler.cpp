...
{
  if (theBehaviorStatus.policyIndex == 333) { // near_opp_goal
    setEyeColor(ledRequest, true, cyan, LEDRequest::fastBlinking);
  } else if (theBehaviorStatus.onBallSkillIndex == 1) { // walkAtRelativeSpeed (dribble)
    setEyeColor(ledRequest, true, yellow, LEDRequest::on);
  } else if (theBehaviorStatus.onBallSkillIndex == 2) { // walkToBallAndKick (kick)
    setEyeColor(ledRequest, true, red, LEDRequest::on);
  } else if (theBehaviorStatus.onBallSkillIndex == 3) { // walkToPose (walking)
    setEyeColor(ledRequest, true, green, LEDRequest::on);
  }
  else if(!theGroundContactState.contact/* && (theFrameInfo.time & 512)*/) { //no groundContact
    setEyeColor(ledRequest, true, yellow, LEDRequest::on);
  }
  else
  {
...
{
  if (theBehaviorStatus.policyIndex == 333) { // near_opp_goal
    setEyeColor(ledRequest, false, cyan, LEDRequest::fastBlinking);
  } else if (theBehaviorStatus.onBallSkillIndex == 1) { // walkAtRelativeSpeed (dribble)
  ...
  else if(Role::isActiveRole(theStrategyStatus.role)) {
    setEyeColor(ledRequest, false, red, LEDRequest::on);
  }
  else
  {
...
void LEDHandler::setHead(LEDRequest& ledRequest)
{
  ...
  else if (theBehaviorStatus.policyIndex == 111) { // off_ball
  ...
  else if (theBehaviorStatus.policyIndex == 222) { // on_ball
    for(LEDRequest::LED i = LEDRequest::firstHeadLED; i <= LEDRequest::lastHeadLED; i = LEDRequest::LED(unsigned(i) + 1))
      ledRequest.ledStates[i] = LEDRequest::blinking;
  }
  else if (theBehaviorStatus.policyIndex == 333) { // near_opp_goal
    for(LEDRequest::LED i = LEDRequest::firstHeadLED; i <= LEDRequest::lastHeadLED; i = LEDRequest::LED(unsigned(i) + 1))
